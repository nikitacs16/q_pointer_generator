...
def attention_decoder(decoder_inputs, initial_state, encoder_states, enc_padding_mask, query_states, que_padding_mask,cell, initial_state_attention=False, pointer_gen=True, use_coverage=False, prev_coverage=None, prev_q_coverage=None):
	...
	Args:
		...
		query_states: 3D Tensor [batch_size x q_attn_length x q_attn_size].
		que_padding_mask: 2D Tensor [batch_size x q_attn_length] containing 1s and 0s; indicates which of the encoder locations are padding (0) or a real token (1)
	...
	with variable_scope.variable_scope("attention_decoder") as scope:
		...
		if use_coverage:
			...
			with variable_scope.variable_scope("q_coverage"):
				w_c_q = variable_scope.get_variable("w_c_q", [1, 1, 1, q_attention_vec_size])

		if prev_coverage is not None: # for beam search mode with coverage
			# reshape from (batch_size, attn_length) to (batch_size, attn_len, 1, 1)
			prev_coverage = tf.expand_dims(tf.expand_dims(prev_coverage,2),3)

		if prev_q_coverage is not None: # for beam search mode with coverage
			# reshape from (batch_size, attn_length) to (batch_size, attn_len, 1, 1)
			prev_q_coverage = tf.expand_dims(tf.expand_dims(prev_q_coverage,2),3)
		...
		def attention(decoder_state, coverage=None):
			...
			with variable_scope.variable_scope("Attention"):
				...
				def masked_attention(e):
					...
					attn_dist = nn_ops.softmax(e*enc_padding_mask) # take softmax. shape (batch_size, attn_length)
		...
		def query_attention(decoder_state, q_coverage=None):
			...
			Args:
				decoder_state: state of the decoder
				coverage: Optional. Previous timestep's coverage vector, shape (batch_size, attn_len, 1, 1). #Never used/

			Returns:
				q_context_vector: weighted sum of encoder_states
				q_attn_dist: attention distribution 
				q_coverage: new coverage vector. shape (batch_size, attn_len, 1, 1) 
			"""
			with variable_scope.variable_scope("QueryAttention"):
				# Pass the decoder state through a linear layer (this is W_s s_t + b_attn in the paper)
				decoder_features = linear(decoder_state, q_attention_vec_size, True) # shape (batch_size, q_attention_vec_size)
				decoder_features = tf.expand_dims(tf.expand_dims(decoder_features, 1), 1) # reshape to (batch_size, 1, 1, attention_vec_size)

				def masked_attention(e):
					"""Take softmax of e then apply enc_padding_mask and re-normalize"""
					attn_dist = nn_ops.softmax(e*que_padding_mask) # take softmax. shape (batch_size, attn_length)
					#attn_dist *= que_padding_mask # apply mask
					masked_sums = tf.reduce_sum(attn_dist, axis=1) # shape (batch_size)
					return attn_dist / tf.reshape(masked_sums, [-1, 1]) # re-normalize
				
				if use_coverage and q_coverage is not None: # non-first step of coverage
					# Multiply coverage vector by w_c to get coverage_features.
					q_coverage_features = nn_ops.conv2d(q_coverage, w_c_q, [1, 1, 1, 1], "SAME") # c has shape (batch_size, attn_length, 1, attention_vec_size)

					# Calculate v^T tanh(W_q q_i + W_s s_t + w_c c_i^t + b_attn)
					f = math_ops.reduce_sum(v_q * math_ops.tanh(query_features + decoder_features + q_coverage_features), [2, 3])  # shape (batch_size,attn_length)

					# Calculate attention distribution
					q_attn_dist = masked_attention(f)

					# Update coverage vector
					q_coverage += array_ops.reshape(q_attn_dist, [batch_size, -1, 1, 1])
				else:
				
					# Calculate v^T tanh(W_q q_i + W_s s_t + b_attn)
					f = math_ops.reduce_sum(v_q * math_ops.tanh(query_features + decoder_features), [2, 3]) # calculate e
				...
				q_context_vector = array_ops.reshape(q_context_vector, [-1, q_attn_size])

			return q_context_vector, q_attn_dist, q_coverage
		...
		q_coverage = prev_q_coverage
		context_vector = array_ops.zeros([batch_size, attn_size])
		context_vector.set_shape([None, attn_size])  # Ensure the second shape of attention vectors is set.
		q_context_vector = array_ops.zeros([batch_size, q_attn_size])
		q_context_vector.set_shape([None, q_attn_size])  # Ensure the second shape of attention vectors is set.
		...
		if initial_state_attention: # true in decode mode
			...
			q_context_vector, _, q_coverage = query_attention(initial_state, q_coverage) #what to do about the initial state of decoder ?
		...
		for i, inp in enumerate(decoder_inputs):
			...
			x = linear([inp] + [context_vector] + [q_context_vector], input_size, True) #is this correct ? Should q_context_vector be removed ?
			...
			if pointer_gen:
				with tf.variable_scope('calculate_pgen'):
					p_gen = linear([context_vector, q_context_vector, state.c, state.h, x], 1, True) # Tensor shape (batch_size, 1)
		...
		return outputs, state, attn_dists, p_gens, coverage, q_coverage
